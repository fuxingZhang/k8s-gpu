...
VERSION=1.0  # MUST modify it for every branch!
...
function api::build::generate() {
  ...
  (
    ...
    # docker build -t gpu-admission-arm64 \
    #   --build-arg version=${VERSION} \
    #   --build-arg commit=${GITCOMMIT} \
    #   --platform=linux/arm64 \
    #   .
    docker buildx build -t gpu-admission-arm64 \
      --build-arg version=${VERSION} \
      --build-arg commit=${GITCOMMIT} \
      --platform=linux/arm64 \
      -o type=docker,dest=gpu-admission-arm64.tar.gz .
  )
}
